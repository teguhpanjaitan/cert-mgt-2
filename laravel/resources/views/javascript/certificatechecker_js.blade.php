...
<?php endif; ?>
	...
	var CertificateCheckerControl = function(){
		
		//Private
		
		//call backs
		var pcallback_RetrieveCertificateComplete = function(status,data){
			CertificateCheckerControl.callback_RetrieveCertificateComplete(status,data);
		};
		...
		return{
			...
			InitCertificateCheckForm: function(jqSelect) {
				...
				jQuery('.btn-check-certificate',formElement).on('click',function(){
					...
					if(formElement.valid()){
						...
						window.location = "/certificate-verification/?certificateno=" + certificatenumber;
			...
			RetrieveCertificate: function(certificatenumber){
				...
				jQuery.post("<?php echo url('/') ?>/certificate/ajax", postData, function(data) {
